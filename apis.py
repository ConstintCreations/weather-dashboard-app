...
def get_weather(settings):
    return requests.get(f"https://api.open-meteo.com/v1/forecast?latitude={settings['latitude']}&longitude={settings['longitude']}&daily=temperature_2m_max,temperature_2m_min,sunset,sunrise,precipitation_sum,precipitation_probability_max&current=apparent_temperature,temperature_2m,is_day,weather_code&hourly=temperature_2m,apparent_temperature&timezone={settings['timezone']}&past_days=7&forecast_days=16&temperature_unit=fahrenheit&precipitation_unit=inch", timeout=8).json()